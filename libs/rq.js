/*
    ...
    2017-04-04
    Public Domain
    snapshot: https://github.com/douglascrockford/RQ/tree/817dd83856ad3bd5e1c59ae7988c9ae4c6c34ba7
...
var RQ = (function () {
  ...
  function check(method, requestors, milliseconds) {
    ...
    if (milliseconds !== undefined && (typeof milliseconds !== "number" || milliseconds < 0)) {
      throw {name: "TypeError", message: "milliseconds", method: method, value: milliseconds};
    }
  }
  ...
  return {
    fallback: function fallback(requestors, milliseconds) {
      ...
      // that will call them each in order until it finds a successful outcome. If all
      // of the requestor functions fail, then the fallback fails. If the time
        ...
        (function next(index, failure) {
          if (typeof callback === "function") {
            ...
            setImmediate(function () {
              var once = true;
              if (typeof callback === "function") {
                cancellation = next_requestor(function next_callback(success, failure) {
                  if (once) {
                    once = false;
                    cancellation = undefined;
    ...
    parallel: function parallel(requireds, milliseconds, optionals, untilliseconds) {
      ...
      // array collecting the results of all of the requestors. RQ.parallel can also
      // take an object of requests, and an optional object of optional requestors. In
      // this case, the result is an object collecting the results of all of the
      // requestors as named properties. If there is no milliseconds argument, then
      ...
      if (typeof requireds === "object" && !Array.isArray(requireds)) {
        ...
        if (optionals) {
          if (optionals === "object" && Array.isArray(optionals)) {
            throw {name: "TypeError", message: "object expected", method: "RQ.parallel", value: optionals};
          }
      ...
      if (optionals) {
        ...
        if (optionals.length === 0) {
          throw {name: "TypeError", message: "optionals expected", method: "RQ.parallel", value: optionals};
        }
      }
      ...
      return function requestor(callback, initial) {
        ...
        if (milliseconds) {
          timeout_id = setTimeout(function () {
            timeout_id = undefined;
            return (requireds_remaining === 0 && (requireds_length > 0 || optionals_successes > 0))
              ? finish(results)
        ...
        if (requireds) {
          requireds
            .forEach(function (requestor, index) {
              return setImmediate(function () {
                var once = true;
                var cancellation = requestor(function parallel_callback(success, failure) {
                  if (once && cancels) {
                    once = false;
                    ...
                    if (requireds_remaining === 0 && !timeout_until) {
                      return finish(results);
                    }
                  }
                }, initial);
        ...
        if (optionals) {
          ...
          optionals.forEach(function (requestor, index) {
            return setImmediate(function () {
              var once = true;
              var cancellation = requestor(function optional_callback(success, failure) {
                if (once && cancels) {
                  once = false;
                  cancels[requireds_length + index] = undefined;
                  if (!failure) {
                    results[requireds_length + index] = success;
                    optionals_successes += 1;
                  }
                  optionals_remaining -= 1;
                  if (optionals_remaining === 0) {
                    if (requireds_remaining === 0) {
                      return (requireds_length > 0 || optionals_successes > 0)
                        ? finish(results)
                        : cancel(failure);
                    }
                    if (timeout_until) {
              ...
              if (cancels[requireds_length + index] === undefined) {
    ...
    race: function race(requestors, milliseconds) {
      ...
      // immediately. The first to finish wins. A race is successful if any contestant
      // is successful. It fails if all requestors fail or if the time expires.
      ...
      return function requestor(callback, initial) {
        ...
        requestors
          .forEach(function (requestor, index) {
            return setImmediate(function () {
              var once = true;
              var cancellation = requestor(function race_callback(success, failure) {
                if (once && cancels) {
                  once = false;
                  cancels[index] = undefined;
                  if (!failure) {
    ...
    sequence: function sequence(requestors, milliseconds) {
      ...
      // is the previous success result. If any of the requestor functions fail, then
      // the whole sequence fails, and the remaining requestors are not called.
      ...
      return function requestor(callback, initial) {
        ...
        (function next(index) {
          ...
          if (typeof next_callback === "function") {
            ...
            setImmediate(function () {
              var once = true;
              cancellation = next_requestor(function sequence_callback(success, failure) {
                if (once) {
                  once = false;
                  cancellation = undefined;
                  if (failure) {
...
exports.requestorize = func => (callback, value) => callback(func(value))

exports.delay = milliseconds => {
  return function requestor(callback, value) {
    var timeout_id = setTimeout(() => callback(value), milliseconds)
